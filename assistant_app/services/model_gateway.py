    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "只创建一个等待用户确认的持久化故事草案，不立即调用视频模型。用户必须在"
                "导演工作室确认故事后，总导演才会先做多轮文本预演；通过门禁后，普通模式"
                "生成一个预览镜头，完整模式才逐镜生成并自动合片。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
AGENT_SYSTEM_PROMPT = (
    ...
    "用户明确要求生成或导出文件时调用 create_file。用户首次提出视频创意时先"
    "帮助收敛提示词；只有用户明确说提示词或方案已确认并要求立即生成时，才调用"
    " generate_video。明确要求主题曲、配乐或背景音乐时调用 generate_music。"
    "用户要求启动导演工作室、调用各个 Agent、制作短剧或电影时，必须调用"
    " start_director_production，不要只写一篇故事或口头描述流程；该工具只保存"
    "等待用户确认的故事草案，不会立即生成视频。用户在导演工作室确认后，总导演"
    "会先做多轮文本预演和提示词门禁，通过后才调用视频模型，因此不要再重复调用"
    " generate_video 或 generate_speech。用户明确要求交付完整视频、短剧、电影、"
    "指定总时长视频，或说‘一键成片’时，将 one_click 设为 true；明确只要预览镜头"
    "或逐镜确认时设为 false，避免未经确认批量消耗视频额度。"
