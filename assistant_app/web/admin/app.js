...
function resetSpeechChannelForm(){ $('#speech-channel-form').reset();$('#speech-channel-id').value='';$('#speech-channel-name').value='MiniMax Speech';$('#speech-channel-url').value='https://api.minimaxi.com';$('#speech-channel-model').value='speech-2.8-hd';$('#speech-channel-voice').value='male-qn-qingse';$('#speech-channel-qps').value='1';$('#speech-channel-format').value='mp3';$('#speech-channel-key').required=true;$('#speech-channel-key').placeholder='sk-...';$('#speech-channel-active').disabled=false;$('#speech-channel-save').textContent='新增语音渠道';$('#speech-channel-cancel').classList.add('hidden'); }
async function loadSpeechChannels(){try{speechChannelsCache=await api('/speech-channels');$('#speech-channel-warning').classList.toggle('hidden',speechChannelsCache.some(item=>item.is_active));$('#speech-channel-grid').innerHTML=speechChannelsCache.map(item=>`<article class="channel-card ${item.is_active?'active':''}"><div class="card-head"><div><h3>${escapeHtml(item.name)}</h3><span class="badge ${item.is_active?'active':'inactive'}">${item.is_active?'已启用':'未启用'}</span></div><span>◉</span></div><div class="channel-url" title="${escapeHtml(item.base_url)}">${escapeHtml(item.base_url)}</div><div class="channel-meta"><div><small>模型</small><strong>${escapeHtml(item.model_name)}</strong></div><div><small>默认声线</small><strong>${escapeHtml(item.default_voice_id)}</strong></div><div><small>QPS 上限</small><strong>${item.qps_limit}</strong></div><div><small>音频格式</small><strong>${item.default_format.toUpperCase()}</strong></div></div><div class="card-actions"><button type="button" class="small-btn" data-speech-edit="${item.id}">编辑</button>${item.is_active?`<button type="button" class="small-btn danger" data-speech-disable="${item.id}">停用渠道</button>`:`<button type="button" class="small-btn activate" data-speech-activate="${item.id}">启用渠道</button>`}</div></article>`).join('')||'<p class="muted">尚未配置语音配音渠道。</p>';}catch(error){notify(error.message);}}
$('#speech-channel-form').addEventListener('submit',async(event)=>{event.preventDefault();const id=$('#speech-channel-id').value;const desiredActive=$('#speech-channel-active').checked;const previous=speechChannelsCache.find(item=>item.id===id);const payload={name:$('#speech-channel-name').value,base_url:$('#speech-channel-url').value,model_name:$('#speech-channel-model').value,default_voice_id:$('#speech-channel-voice').value,qps_limit:Number($('#speech-channel-qps').value),default_format:$('#speech-channel-format').value};if(!id||$('#speech-channel-key').value)payload.api_key=$('#speech-channel-key').value;if(!id)payload.is_active=desiredActive;try{await api(id?`/speech-channels/${id}`:'/speech-channels',{method:id?'PATCH':'POST',body:JSON.stringify(payload)});if(id&&previous&&previous.is_active!==desiredActive)await api(`/speech-channels/${id}/${desiredActive?'activate':'disable'}`,{method:'POST'});notify(id?`语音渠道已更新${desiredActive?'并启用':'并停用'}`:`语音渠道已新增${desiredActive?'并启用':''}`);resetSpeechChannelForm();await loadSpeechChannels();}catch(error){notify(error.message);}});
$('#speech-channel-grid').addEventListener('click',async(event)=>{const edit=event.target.closest('[data-speech-edit]');const activate=event.target.closest('[data-speech-activate]');const disable=event.target.closest('[data-speech-disable]');try{if(edit){const item=speechChannelsCache.find(row=>row.id===edit.dataset.speechEdit);$('#speech-channel-id').value=item.id;$('#speech-channel-name').value=item.name;$('#speech-channel-url').value=item.base_url;$('#speech-channel-model').value=item.model_name;$('#speech-channel-voice').value=item.default_voice_id;$('#speech-channel-qps').value=item.qps_limit;$('#speech-channel-format').value=item.default_format;$('#speech-channel-key').value='';$('#speech-channel-key').required=false;$('#speech-channel-key').placeholder='留空则保持原 Key';$('#speech-channel-active').checked=item.is_active;$('#speech-channel-active').disabled=false;$('#speech-channel-save').textContent='保存修改';$('#speech-channel-cancel').classList.remove('hidden');scrollTo({top:0,behavior:'smooth'});return;}if(activate){await api(`/speech-channels/${activate.dataset.speechActivate}/activate`,{method:'POST'});notify('语音渠道已启用');}if(disable){await api(`/speech-channels/${disable.dataset.speechDisable}/disable`,{method:'POST'});notify('语音渠道已停用');}await loadSpeechChannels();}catch(error){notify(error.message);}});
$('#speech-channel-cancel').addEventListener('click',resetSpeechChannelForm);
