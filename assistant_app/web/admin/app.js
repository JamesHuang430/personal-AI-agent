...
$('#video-channel-form').addEventListener('submit',async(event)=>{event.preventDefault();const id=$('#video-channel-id').value;const payload={name:$('#video-channel-name').value,provider:$('#video-channel-provider').value,base_url:$('#video-channel-url').value,model_name:$('#video-channel-model').value,qps_limit:Number($('#video-channel-qps').value),default_seconds:$('#video-channel-seconds').value,default_size:$('#video-channel-size').value,default_resolution:$('#video-channel-resolution').value};if(!id||$('#video-channel-key').value)payload.api_key=$('#video-channel-key').value;if(!id)payload.is_active=$('#video-channel-active').checked;try{await api(id?`/video-channels/${id}`:'/video-channels',{method:id?'PATCH':'POST',body:JSON.stringify(payload)});notify(id?'视频渠道已更新':'视频渠道已新增');resetVideoChannelForm();await loadVideoChannels();}catch(error){notify(error.message);}});
...
function resetMusicChannelForm(){ $('#music-channel-form').reset();$('#music-channel-id').value='';$('#music-channel-name').value='MiniMax Music';$('#music-channel-url').value='https://api.minimax.io';$('#music-channel-model').value='music-2.6';$('#music-channel-qps').value='1';$('#music-channel-format').value='mp3';$('#music-channel-key').required=true;$('#music-channel-key').placeholder='sk-...';$('#music-channel-active').disabled=false;$('#music-channel-save').textContent='新增音乐渠道';$('#music-channel-cancel').classList.add('hidden'); }
async function loadMusicChannels(){try{musicChannelsCache=await api('/music-channels');$('#music-channel-grid').innerHTML=musicChannelsCache.map(item=>`<article class="channel-card ${item.is_active?'active':''}"><div class="card-head"><div><h3>${escapeHtml(item.name)}</h3><span class="badge ${item.is_active?'active':'inactive'}">${item.is_active?'当前音乐渠道':'未启用'}</span></div><span>♫</span></div><div class="channel-url" title="${escapeHtml(item.base_url)}">${escapeHtml(item.base_url)}</div><div class="channel-meta"><div><small>模型</small><strong>${escapeHtml(item.model_name)}</strong></div><div><small>QPS 上限</small><strong>${item.qps_limit}</strong></div><div><small>音频格式</small><strong>${item.default_format.toUpperCase()}</strong></div></div><div class="card-actions"><button class="small-btn" data-music-edit="${item.id}">编辑</button>${item.is_active?`<button class="small-btn danger" data-music-disable="${item.id}">停用</button>`:`<button class="small-btn" data-music-activate="${item.id}">设为当前</button>`}</div></article>`).join('')||'<p class="muted">尚未配置音乐生成渠道。</p>';}catch(error){notify(error.message);}}
$('#music-channel-form').addEventListener('submit',async(event)=>{event.preventDefault();const id=$('#music-channel-id').value;const payload={name:$('#music-channel-name').value,base_url:$('#music-channel-url').value,model_name:$('#music-channel-model').value,qps_limit:Number($('#music-channel-qps').value),default_format:$('#music-channel-format').value};if(!id||$('#music-channel-key').value)payload.api_key=$('#music-channel-key').value;if(!id)payload.is_active=$('#music-channel-active').checked;try{await api(id?`/music-channels/${id}`:'/music-channels',{method:id?'PATCH':'POST',body:JSON.stringify(payload)});notify(id?'音乐渠道已更新':'音乐渠道已新增');resetMusicChannelForm();await loadMusicChannels();}catch(error){notify(error.message);}});
$('#music-channel-grid').addEventListener('click',async(event)=>{const edit=event.target.closest('[data-music-edit]');const activate=event.target.closest('[data-music-activate]');const disable=event.target.closest('[data-music-disable]');try{if(edit){const item=musicChannelsCache.find(row=>row.id===edit.dataset.musicEdit);$('#music-channel-id').value=item.id;$('#music-channel-name').value=item.name;$('#music-channel-url').value=item.base_url;$('#music-channel-model').value=item.model_name;$('#music-channel-qps').value=item.qps_limit;$('#music-channel-format').value=item.default_format;$('#music-channel-key').value='';$('#music-channel-key').required=false;$('#music-channel-key').placeholder='留空则保持原 Key';$('#music-channel-active').checked=item.is_active;$('#music-channel-active').disabled=true;$('#music-channel-save').textContent='保存修改';$('#music-channel-cancel').classList.remove('hidden');scrollTo({top:0,behavior:'smooth'});return;}if(activate){await api(`/music-channels/${activate.dataset.musicActivate}/activate`,{method:'POST'});notify('已切换当前音乐渠道');}if(disable){await api(`/music-channels/${disable.dataset.musicDisable}/disable`,{method:'POST'});notify('音乐渠道已停用');}await loadMusicChannels();}catch(error){notify(error.message);}});
