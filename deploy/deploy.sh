...
"${compose_cmd[@]}" up -d postgres redis assistant-api
...
for attempt in $(seq 1 30); do
    if curl --silent --show-error --fail \
        "http://127.0.0.1:${ASSISTANT_API_PORT:-18000}/api/v1/health/ready" >/dev/null; then
        echo "API is ready."
        "${compose_cmd[@]}" ps
        exit 0
    fi
    if [[ "${attempt}" -eq 30 ]]; then
        ...
        "${compose_cmd[@]}" ps
        "${compose_cmd[@]}" logs --tail=200 assistant-api postgres redis
        exit 1
    fi
    sleep 2
done
