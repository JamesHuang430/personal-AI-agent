...
> 历史方案，已于 2026-09-05 收敛为视频创作产品。天气、票务、通用知识平台、
> 扩展市场等不再是当前交付范围。当前需求以 [视频创作产品方案](video-product-focus.md) 为准。
...
长期记忆写入采用三种模式：`off`、`ask`、`auto_after_session`。默认使用 `auto_after_session`：每轮问答只做低延迟的增量提取；当用户点击“结束并整理”、新建对话或切换到另一段会话时，再对整段会话做一次归纳，去重后写入向量数据库和个人知识图谱，并明确通知用户本次识别出的记忆、实体和关系数量。原始对话与长期记忆分开保存，用户可查看和删除任意记忆。
